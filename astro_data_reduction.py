...
aligned_cube[~np.isfinite(aligned_cube)] = np.nan               # Wherever the cube contains a non-finite value, replace it with NaN.

clipped_cube = sigma_clip(
    aligned_cube,           # input data
    sigma=3.0,              # Mask values farther than 3 standard deviations from the center.
    maxiters=5,             # Repeat clipping up to 5 times, stopping early if no new outliers are found
    cenfunc='median',       # use the median as the center estimate
    stdfunc='std',          # use the standard deviation as the measure of spread
    axis=0,                 # clip along the image stack axis
    masked=True             # return a masked array. So if one value is clipped, it is not erased; it is marked as masked.
)

final_img_stacked = np.ma.mean(clipped_cube, axis=0).filled(np.nan) # Average the clipped stack, ignoring rejected pixels, and represent any remaining invalid output pixels as NaN
